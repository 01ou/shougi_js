...
class Board {
    ...
    if (boardElement) {
      for (let row = 0; row < BOARD_SIZE; row++) {
        ...
        for (let col = 0; col < BOARD_SIZE; col++) {
          ...
          const [x, y] = Board.convertPositionToIndex(position)
  ...
  static convertPositionToIndex(position) {
    const x = position[1];
...
class PieceManager {
  ...
  static initializeSetPiece() {
    const set = (type, position) => {
      const reveresPosition = reversingPosition(position, true, false);
      const opponentPosition = reversingPosition(position, false, true);
      const reveresOpponentPosition = reversingPosition(position, true, true);
      cellManager.setNewPiece(type, position, false);
      cellManager.setNewPiece(type, reveresPosition, false);
      cellManager.setNewPiece(type, opponentPosition, true);
      cellManager.setNewPiece(type, reveresOpponentPosition, true);
    };
    ...
    const reversingPosition = (position, reveresX=false, reveresY=false) => {
  ...
  static checkCanMoveInsideBoard(piece, position, removeOverlap) {
    if (!piece || !position) {
      return false;
    }
  
    const moveables = piece.getNormalMoveable(position, removeOverlap);
  ...
  addClickListener() {
    this.element.addEventListener("click", () => {
      ...
      if (this.piece && this.piece.team == nowTeam) {
        ...
        cellManager.resetCellsState();
        cellManager.setCellsMoveableState(moveablePositions);
      ...
      // 駒を移動する
      cellManager.movePieceAtCell(nowSelectCell, this);
    });
  }
...
class CellManager {
  ...
  initializeCells() {
    for (let x = 0; x < BOARD_SIZE; x++) {
      for (let y = 0; y < BOARD_SIZE; y++) {
        const [row, col] = Board.convertPositionToIndex([x, y])
  ...
  movePieceAtCell(beforeCell, targetCell) {
    ...
    if (canPromote) {
      const isCanMove = PieceManager.checkCanMoveInsideBoard(beforePiece, targetCell.position, false);
      if (isCanMove) {
  ...
  checkIncludePiece(position) {
    ...
    const isIncludeAlly = piece ? piece.team == nowTeam : false;

    return [isInclude, isIncludeAlly];
  }
...
class Piece {
  ...
  adjustPosition(position, currentPosition, removeOverlap = true, isList = false) {
    ...
    if (removeOverlap) {
      posList = this.removeOverlapsAlly(posList);
    }
  ...
  removeOverlapsAlly(positions) {
    let posList = [];
    positions.forEach(pos => {
      const [, isIncludeAlly] = cellManager.checkIncludePiece(pos);
      if (pos && !isIncludeAlly) {
        posList.push(pos);
      }
    });
    return posList;
  ...
  around(currentPosition = this.position, removeOverlap = true) {
    let moveableList = [];
    for (let x = -1; x < 2; x++) {
      for (let y = -1; y < 2; y++) {
        if (x !== 0 || y !== 0) {
          const pos = this.adjustPosition([x, y], currentPosition, removeOverlap);
  ...
  // 直線方向の移動可能な位置を返すメソッド
  straight(directions, currentPosition = this.position, removeOverlap = true) {
    let moveableList = [];
    ...
    for (const direction in directions) {
      ...
      while (loopCount < maxCount) {
        loopCount++;
        const pos = this.adjustPosition([x * loopCount, y * loopCount], currentPosition, removeOverlap);
        const [isInclude, isIncludeAlly] = cellManager.checkIncludePiece(pos);
  ...
  // 前方の移動可能な位置を返すメソッド
  front(directions, currentPosition = this.position, removeOverlap = true) {
    let moveableList = [];
    for (const direction in directions) {
      const { x, y } = directions[direction];
      const pos = this.adjustPosition([x, y], currentPosition, removeOverlap);
...
class King extends Piece {
  ...
  getNormalMoveable(currentPosition = this.position, removeOverlap = true) {
    return this.around(currentPosition, removeOverlap);
  }
}
...
class Bishop extends Piece {
  ...
  getNormalMoveable(currentPosition = this.position, removeOverlap = true) {
    ...
    return this.straight(moveDirections, currentPosition, removeOverlap);
  }
}
...
class Gold extends Piece {
  ...
  getNormalMoveable(currentPosition = this.position, removeOverlap = true) {
    const moveDirections = [DIRECTIONS.UP, DIRECTIONS.UPRIGHT, DIRECTIONS.UPLEFT, DIRECTIONS.RIGHT, DIRECTIONS.LEFT, DIRECTIONS.DOWN];
    return this.front(moveDirections, currentPosition, removeOverlap);
  }
}
...
class Knight extends Piece {
  ...
  getNormalMoveable(currentPosition = this.position, removeOverlap = true) {
    ...
    return this.adjustPosition(moveableList, currentPosition, removeOverlap, true);
  }
}
...
const addHoldPieceToNowTeam = (piece) => {
  ...
  switch (nowTeam) {
    case false:
      teamFalseHoldPiece.push(newPiece);
      break;
    case true:
...
const removePieceFromHold = (removePiece) => {
  ...
  switch (team) {
    case false:
      teamFalseHoldPiece = teamFalseHoldPiece.filter(piece => piece != removePiece);
      break;
    case true:
...
const updateDisplayHoldPieces = () => {
  createHoldPiecesElement('team-false-hold-piece', teamFalseHoldPiece);
...
const initializeGame = () => {
  Board.createBoard(cellManager.cells);
